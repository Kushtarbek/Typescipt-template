...
const ProjectDescription: FC<ProjectDescriprionProps> = ({ className, onBack, onComplete, ...rest }) => {
    ...
    <form onSubmit={handleSubmit} className={clsx(classes.root, className)} {...rest}>
      ...
      <Box mt={2}>
        <Typography variant="subtitle1" color="textSecondary">
          Select a Poshmark campaign type
        </Typography>
      </Box>
      <Paper className={classes.editorContainer} variant="outlined">
        <QuillEditor handleChange={handleChange} value={content} className={classes.editor} />
      </Paper>
      {error && (
        <Box mt={2}>
