...
const ThreadItem: FC<ThreadItemProps> = ({ active, className, thread, onSelect, ...rest }) => {
  ...
  return (
    <ListItem button className={clsx({ [classes.active]: active }, className)} onClick={onSelect} {...rest}>
      <ListItemAvatar>
        <AvatarGroup classes={{ avatar: details.otherParticipants.length > 1 ? classes.smallAvatar : null }} max={2}>
          {details.otherParticipants.map((participant) => (
            <Avatar alt="Person" key={participant.id} src={participant.avatar} />
          ))}
        </AvatarGroup>
      </ListItemAvatar>
      <ListItemText
      ...
      <Box ml={2} display="flex" flexDirection="column" alignItems="flex-end">
        {thread.unreadCount > 0 && (
          <Chip className={classes.unreadIndicator} color="secondary" size="small" label={thread.unreadCount} />
        )}
      </Box>
    </ListItem>
  );
