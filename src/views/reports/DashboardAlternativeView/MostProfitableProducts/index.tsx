...
const MostProfitableProducts: FC<MostProfitableProductsProps> = ({ className, ...rest }) => {
  ...
  return (
    <Card className={clsx(classes.root, className)} {...rest}>
      <CardHeader action={<GenericMoreButton />} title="Most Profitable Products" />
      <Divider />
      <PerfectScrollbar>
        <Box minWidth={700}>
          <Table>
                    ...
                    <Box display="flex" alignItems="center">
                      ...
                      <Box ml={2}>
                        ...
                        <Typography variant="body2" color="textSecondary">
                          <span className={classes.subscriptions}>{numeral(product.subscriptions).format('0,0')}</span>{' '}
                          Active
                        </Typography>
                      </Box>
                    </Box>
      ...
      <Box p={2} display="flex" justifyContent="flex-end">
        <Button component={RouterLink} size="small" to="#" endIcon={<NavigateNextIcon />}>
          See all
        </Button>
      </Box>
    </Card>
