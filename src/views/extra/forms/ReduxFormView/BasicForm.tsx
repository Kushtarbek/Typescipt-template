...
const renderTextField = ({ input, label, meta: { touched, invalid, error }, ...rest }) => {
  return (
    <TextField
...
const renderCheckbox = ({ input, label, meta: { touched, invalid, error }, ...rest }) => {
  return (
    <div>
      <Box alignItems="center" display="flex" ml={-1}>
        <Checkbox checked={!!input.value} onChange={input.onChange} {...input} {...rest} />
        {label}
      </Box>
      {Boolean(touched && invalid) && <FormHelperText error>{error}</FormHelperText>}
    </div>
  );
};
