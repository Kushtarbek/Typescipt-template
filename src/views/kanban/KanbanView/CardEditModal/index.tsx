...
import { deleteCard, updateCard, addChecklist } from 'src/slices/kanban';
...
const CardEditModal: FC<CardEditModalProps> = ({ card, className, list, onClose, open, ...rest }) => {
            ...
            {card.checklists.length > 0 && (
              <Box mt={5}>
                {card.checklists.map((checklist) => (
                  <Checklist key={checklist.id} card={card} checklist={checklist} className={classes.checklist} />
                ))}
              </Box>
            )}
            <Box mt={3}>
