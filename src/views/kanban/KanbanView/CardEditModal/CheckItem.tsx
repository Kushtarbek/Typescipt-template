...
import type { Card, Checklist, CheckItem as CheckItemType } from 'src/types/kanban';
...
const CheckItem: FC<CheckItemProps> = ({
  ...
  return (
    <div className={clsx(classes.root, className)} {...rest}>
      <Checkbox checked={checkItem.state === 'complete'} onChange={handleStateChange} className={classes.checkbox} />
      {editing ? (
        <Box flexGrow={1}>
          <TextField value={name} variant="outlined" fullWidth onChange={handleNameChange} />
          <Box mt={1}>
        ...
        <Box display="flex" alignItems="center" flexGrow={1}>
          <Typography onClick={onEditInit} color="textPrimary" variant="body1" className={classes.name}>
            {checkItem.name}
          </Typography>
