...
import type { Card as CardType, List, Member } from 'src/types/kanban';
...
const Card: FC<CardProps> = forwardRef(({ cardId, className, dragging, index, list, style, ...rest }, ref) => {
  ...
  return (
    ...
    >
      <MuiCard
        ...
        onClick={handleOpen}
      >
        {card.cover && <CardMedia className={classes.cover} image={card.cover} />}
        <CardContent>
          ...
          <Box mt={2} display="flex" alignItems="center">
            {card.isSubscribed && (
              <SvgIcon className={classes.badge} color="action" fontSize="small">
                <EyeIcon />
              </SvgIcon>
            )}
      ...
      <CardEditModal open={isOpened} onClose={handleClose} card={card} list={list} />
    </div>
  );
});
